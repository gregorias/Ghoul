...
public class KademliaApp extends App<Environment> {
  ...
  public synchronized void startUp() throws IOException {
    LOGGER.debug("[{} {}] startUp()", getId(), mUri);
  ...
  public synchronized void shutDown() throws IOException {
    LOGGER.debug("[{} {}] shutDown()", getId(), mUri);
  ...
  private Response sendRequest(String path, HTTPType type) throws IOException {
    try {
      Client client = ClientBuilder.newClient();
      ...
      throw new IOException("Could not send HTTP request.", e);
    }
  }
}
