...
It initiates and maintains its routing table and answers queries.
Additionally, each DHT node is responsible for maintaining the security of the
entire network by maintaining its security certificate and responding to
security queries.
...
\nextlevel
\referencestart{c}{the challenge protocol}{client}{ra}
\nextlevel[5]
...
\label{sec:ghoul_summary}
  ...
  Ghoul changed just node join and neighbor additions protocols and as
  ...
  Tor anonymizing network \cite{syv04}, which is used worldwide by, for example,
